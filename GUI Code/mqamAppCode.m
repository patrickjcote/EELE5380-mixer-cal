classdef mqamApp < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function SendButtonValueChanged(app, event)
            ...
            dataBlock = buildDataBlock(app);
            
            try
                buildMQAM(M,Fsym,dataBlock,TX_CAL,2,AWGVisaType,AWGVisaAddr);
        ...
        function ReadButtonValueChanged(app, event)
            ...
            try
                [rxBits, rxLLRs] = readMQAM(M,Fsym,N_syms,dataBlock,RX_CAL,DSOVisaType,DSOVisaAddr);
                app.Status.FontColor = [0.47 0.67 0.19];
...
endclassdef mqamApp < matlab.apps.AppBase
